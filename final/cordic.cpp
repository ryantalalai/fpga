...
static FIXED_POINT K = 0.60735;  // Scaling Factor
...
void cordic_rotator(FIXED_POINT cos, FIXED_POINT sin, FIXED_POINT *mag, FIXED_POINT *theta) {
	    ...
	    FIXED_POINT current_phase = 0;
	    FIXED_POINT ninety = 1.570796;
	    FIXED_POINT one_eighty = 3.141592;
	    ...
	    if (current_cos < 0) {
	            if (current_sin >= 0) {
	                ...
	                current_sin = temp;
	                current_phase -= ninety;
	            } else {
	                current_cos = -current_cos;
	                current_sin = -current_sin;
	                current_phase += one_eighty;
	            }
	        } else if (current_sin < 0) {
	        	FIXED_POINT temp = current_cos;
	            current_cos = current_sin;
	            current_sin = -temp;
	            current_phase += ninety;
	        ...
	        for (int j = 0; j < NUM_ITERATIONS; j++) {
	            ...
	            if (current_sin >= 0) {
	                current_cos += sin_shift;
	                current_sin -= cos_shift;
	                current_phase += cordic_phase[j];
	            } else {
	                current_cos -= sin_shift;
	                current_sin += cos_shift;
	                current_phase -= cordic_phase[j];
